...
class Stats(commands.Cog):
    ...
    @app_commands.command(name="주사위", description="주사위를 굴립니다. (기본: 1-100)")
    @app_commands.describe(min_val="최솟값 (기본 1)", max_val="최댓값 (기본 100)")
    async def dice(self, interaction: discord.Interaction, min_val: int = 1, max_val: int = 100):
        ...
        # 주사위 이벤트 발송 (다른 Cog에서 감지 가능)
        self.bot.dispatch("dice_roll", interaction, result)
